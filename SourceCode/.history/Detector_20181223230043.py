...
def detect(image,Evaluator):
    ...
    image_parts_values_normalized = list(p.map(Utils.varianceNormalize,image_parts_values))
    ii_parts_values =  list(p.map(toII,image_parts_values_normalized))
    print("Predicting Started !!!")
    print(len(image_parts_ranges[i]),image_parts_ranges[i])
    all_detected_squares = [(image_parts_ranges[i],image_parts_values[i].shape) for i in image_parts_ranges[i]]
