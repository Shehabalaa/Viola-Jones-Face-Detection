...
def detect1(ii,Evaluator):
    ...
    while(w < ii.shape[0] and w<ii.shape[1]):
        ...
        for img in ii_parts_values:
            cv2.imshow('a', img)
            cv2.waitKey(1)
        ii_parts_values = [cv2.resize(img,(24,24)) for img in ii_parts_values]
        ii_parts_values_normalized = list(map(Utils.varianceNormalize,ii_parts_values))
        ii_parts_values =  list(map(toII,ii_parts_values_normalized))
        all_detected_squares += [ii_parts_ranges[i] for i in Evaluator.predict(ii_parts_values, (1.0,1.0))] #(w/24,h/24)
        offset_w += 1
        offset_h += 1
        if(len(all_detected_squares)):
            w_h_pairs.append((len(all_detected_squares), w,h))
...
def main():
    ...
    while(True):
        ...
        recs,w_h_pairs = detect(gray,Evaluator)
        #recs = detect1(toII(Utils.varianceNormalize(gray)),Evaluator)
        count = 0
        for i in range(len(recs)):
            width = w_h_pairs[count][1]
            height = w_h_pairs[count][2]
            cv2.rectangle(frame,(recs[i][1],recs[i][1]+width),(recs[i][0],recs[i][0]+height), (255, 0, 0), 2)
            if(i >= w_h_pairs[count][0]-1):
                count+=1
...
"""
    ...
    imagnge(len(res)):
        if(res[i]==1):
                doubts.append(pos_of_iis_to_detect[i])
