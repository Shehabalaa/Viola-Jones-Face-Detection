...
class HaarLikeFeature(object):
    ...
    def calcScore(self, ii,feature_scale=(1.0,1.0)):
        ...
        try:
            top_left = tuple(map(int, np.multiply(self.top_left,feature_scale)))
            bottom_right = tuple(map(int, np.multiply(self.bottom_right,feature_scale)))
            width , height = tuple(map(int,np.multiply((self.width,self.height),feature_scale)))
            score = 0
        except ValueError as err:
            print("top_left {0}",top_left)
            print("bottom_right {0}",bottom_right)
            exit(0)
