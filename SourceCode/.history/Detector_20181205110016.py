...
def detect(image,Evaluator):
    ...
    w = 100 # width and height are equals as i will scan image in squares
    h = int(1.25(w))
    offset_w = 10
    offset_h = 10
    image_parts_ranges=[]
    image_parts_values=[]
    while(w < image.shape[0] and w<image.shape[1]):
        r = list(range(0, image.shape[0]-h,offset_h))
        c = list(range(0,image.shape[1]-w,offset_w))
        image_parts_ranges = list(itertools.product(r, c))
        image_parts_values = list(map(lambda p: image[p[0]:p[0]+h, p[1]:p[1]+w],image_parts_ranges))
        ...
        #offset_w += 1
        all_detected_squares += [image_parts_ranges[i] for i in Evaluator.predict(ii_parts_values, (1.0,1.0))] #(w/24,h/24)
        if(len(all_detected_squares)):
            width_pairs.append((len(all_detected_squares), w,h))
        w = int(round(w*1.5))
    return all_detected_squares,width_pairs
        
'''
def detect1(ii,Evaluator):
    all_detected_squares = []
    w = 24 # width and height are equals as i will scan ii in squares
    offset_w = 1
    ii_parts_ranges=[]
    ii_parts_values=[]
    while(w < ii.shape[0] and w<ii.shape[1]):
        r = list(range(0,ii.shape[0]+offset_w, w -1 + offset_w))
        r = [ (i,j-offset_w) for (i,j) in zip(r,r[1:])]
        ...
        #offset_w += 1
        all_detected_squares += [ii_parts_ranges[i] for i in Evaluator.predict(ii_parts_values,w/base_detector_width)]
        w = int(round(w*1.5))
...
def main():
    ...
    while(True):
        ...
        recs,width_pairs = detect(gray,Evaluator)
        #recs = detect1(toII(Utils.varianceNormalize(gray)),Evaluator)
        print(width_pairs)
        count = 0
        for i in range(len(recs)):
            width = width_pairs[count][1]
            height = width_pairs[count][1]
            cv2.rectangle(frame,(recs[i][1],recs[i][1]+width),(recs[i][0],recs[i][0]+width), (255, 0, 0), 2)
            if(i >= width_pairs[count][0]-1):
                count+=1
