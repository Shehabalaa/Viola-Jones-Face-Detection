...
def detect(image,Evaluator):
    ...
    w = 50 # width and height are equals as i will scan image in squares
    h = 50
    offset_w = 10
    offset_h = 10
    image_parts_ranges=[]
    image_parts_values=[]
    while(w<200 and w < image.shape[0] and w<image.shape[1]):
        r = list(range(0, image.shape[0]-h-1,offset_h))
        c = list(range(0,image.shape[1]-w-1,offset_w))
    ...
    print(len(image_parts_ranges))    
    image_parts_values = [cv2.resize(img,(24,24)) for img in image_parts_values]
    image_parts_values_normalized = list(map(Utils.varianceNormalize,image_parts_values))[Utils.varianceNormalize(img) for img in image_parts_values]
    ii_parts_values =  list(map(toII,image_parts_values_normalized))
    print("Predicting Started !!!")
    indices = Evaluator.predict(ii_parts_values)
    print(len(image_parts_ranges),indices)
    print("Predicting Ended !!!")
    all_detected_squares = [(image_parts_ranges[i],image_parts_values[i].shape) for i in indices]
...
def main():
    ...
    while(True):
        ...
        for i in range(len(recs)):
            #[(r,c),(w,h)]
            cv2.rectangle(frame,(recs[i][0][1],recs[i][0][1]+recs[i][1][0]),(recs[i][0][0],recs[i][0][0]+recs[i][1][1]), (255, 0, 0), 2)
