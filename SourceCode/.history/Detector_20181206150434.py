...
def detect(image,Evaluator):
    ...
    while(w < image.shape[0] and w<image.shape[1]):
        ...
        image_parts_ranges = np.array(itertools.product(r, c))
        ...
        pos_indecies = Evaluator.predict(ii_parts_values, (1.0,1.0))
        all_detected_squares += image_parts_ranges[pos_indecies] #(w/24,h/24)
        offset_w += 1
        offset_h += 1
