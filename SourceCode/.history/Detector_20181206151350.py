...
def detect(image,Evaluator):
    ...
    while(w < image.shape[0] and w<image.shape[1]):
        ...
        image_parts_ranges = np.array(list(itertools.product(r, c)))
        ...
        all_detected_squares += list(image_parts_ranges[Evaluator.predict(ii_parts_values, (1.0,1.0))]) #(w/24,h/24)
        offset_w += 1
        offset_h += 1
