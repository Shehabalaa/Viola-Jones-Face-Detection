...
class HaarLikeFeature(object):
    ...
    def calcScore(self, ii,feature_scale=(1.0,1.0)):
        ...
        top_left = map(int,tuple (np.multiply(self.top_left,feature_scale)))
        bottom_right = map(int,tuple (np.multiply(self.bottom_right,feature_scale)))
        width , height = map(int,tuple(np.multiply((self.width,self.height),feature_scale)))
        score = 0
