...
def detect(image,Evaluator):
    ...
    w = 40 # width and height are equals as i will scan image in squares
    h = 40
    offset_w = 5
    offset_h = 5
    image_parts_ranges=[]
...
def main():
    ...
    while(True):
        ...
        recs = [(recs[i][0][0],recs[i][0][0]+recs[i][1][0]),(recs[i][0][1],recs[i][0][1]+recs[i][1][1]) for  i in range(len(recs)]
        recs = nonMaxSuppression(recs,.1)
        for i in range(len(recs)):
            cv2.rectangle(frame,(recs[i][0][0],recs[i][0][0]+recs[i][1][0]),(recs[i][0][1],recs[i][0][1]+recs[i][1][1]), (255, 0, 0), 2)
