...
class HaarLikeFeature(object):
    ...
    def calcScore(self, ii,feature_scale=1.0):
        #Get score for given integral image array.
        top_left = tuple (np.multiply(self.top_left,feature_scale))
        bottom_right = tuple (np.multiply(self.bottom_right,feature_scale))
        height , width = tuple(np.multiply((self.height,self.width),feature_scale))
        score = 0
