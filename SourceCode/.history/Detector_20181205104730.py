...
def detect(image,Evaluator):
    ...
    while(w < image.shape[0] and w<image.shape[1]):
        r = list(range(0, image.shape[0]-w,offset_h))
        c = list(range(0,image.shape[1]-w,offset_w))
        image_parts_ranges = list(itertools.product(r, c))
        image_parts_values = list(map(lambda p: image[p[0]:p[0]+w, p[1]:p[1]+w],image_parts_ranges))
        ...
        #offset_w += 1
        all_detected_squares += [image_parts_ranges[i] for i in Evaluator.predict(ii_parts_values, 1)]
