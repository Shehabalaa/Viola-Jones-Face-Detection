...
def detect(image,Evaluator,w=80,offset_w=15,offset_h=15):
    ...
    while(h < image.shape[0] and w<image.shape[1]):
        ...
        offset_w = max(30,offset_w+1)
        offset_h = max(30,offset_h+1)
        w = int(round(w*1.25))
        h=w
...
def main():
    ...
    print(len(Evaluator.stages[3].weak_classifiers),len(Evaluator.stages[4].weak_classifiers))
    print(len(Evaluator.stages[7].weak_classifiers),len(Evaluator.stages[8].weak_classifiers))
    #tryOnImage('faces.jpg',Evaluator,24)
