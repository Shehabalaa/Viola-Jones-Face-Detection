...
def detectFast(ii,Evaluator):
    ...
    while(w < ii.shape[0] and w<ii.shape[1]):
        ...
        all_detected_squares += [ii_parts_ranges[i] for i in Evaluator.predict(ii_parts_values,(w/24.,h/24.))] #(w/24.,h/24.)
        offset_w += 1
        offset_h += 1
        if(len(all_detected_squares)):
            w_h_pairs.append((len(all_detected_squares), w,h))
...
def main():
    ...
    while(True):
        ...
        #recs,w_h_pairs = detect1(toII(Utils.varianceNormalize(gray)),Evaluator)
        count = 0
