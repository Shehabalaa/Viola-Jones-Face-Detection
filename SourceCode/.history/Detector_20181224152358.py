...
def detect(image,Evaluator,w=80,h=80,offset_w=20,offset_h=20):
    w_h_pairs=[]
    ...
    while(h < image.shape[0] and w<image.shape[1]):
        ...
        offset_w = max(2,offset_w-2)
        offset_h = max(2,offset_h-2)
        w = int(round(w*1.25))
        h = int(round(h*1.25))
    ...
    # ii_parts_values =   
    print("Processing...")
    ii_parts_values =[cv2.integral(Utils.varianceNormalize(cv2.resize(img,(24,24))))[1:,1:] for img in image_parts_values]
    print("Detectiong...")
...
def tryOnImage(img_file,Cascade):
    ...
    if(cv2.waitKey(0) & 0xFF == ord('q')):
        return
    cv2.imwrite('detected'+img_file)



def main():
    Evaluator = Cascade('../Cascade/')
    tryOnImage('faces.jpg')
