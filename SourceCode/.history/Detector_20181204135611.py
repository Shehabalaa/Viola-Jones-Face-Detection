...
def detect(image,Evaluator):
    ...
    while(w < image.shape[0] and w<image.shape[1]):
        r = list(range(0,image.shape[0]+offset_w, w -1 + offset_w))
        r = [ (i,j-offset_w) for (i,j) in zip(r,r[1:])]
        c = list(range(0,image.shape[1]+offset_w, w -1 + offset_w))
        c = [ (i,j-offset_w) for (i,j) in zip(c,c[1:])]
        image_parts_ranges = [rectangle for rectangle in itertools.product(r,c) ]
        image_parts_values = [image[p[0][0]:p[0][1]+1,p[1][0]:p[1][1]+1] for p in image_parts_ranges]
        image_parts_values = list(map(partial(cv2.resize,(24,24)),image_parts_values))
        ...
        #offset_w += 1
        all_detected_squares += [image_parts_ranges[i] for i in Evaluator.predict(ii_parts_values,w/base_detector_width)]
        w = int(round(w*1.5))
...
def main():
    ...
    while(True):
        ...
        #for rec in recs:
        #    cv2.rectangle(frame,(rec[1][0],rec[0][0]),(rec[1][1],rec[0][1]), (255, 0, 0), 2)  
        
        cv2.imshow('frame',Utils.varianceNormalize(gray).astype('uint8'))
