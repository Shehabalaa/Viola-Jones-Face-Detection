...
class HaarLikeFeature(object):
    ...
    def calcScore(self, ii):
        #Get score for given integral image array.
        #top_left = tuple(np.multiply(self.top_left,feature_scale[::-1]))
        #bottom_right = tuple(np.multiply(self.bottom_right,feature_scale[::-1]))
        #width , height = tuple(np.multiply((self.width,self.height),feature_scale))
        top_left=self.top_left
        bottom_right = self.bottom_right
        width,height = self.weight,self.height
        score = 0
    ...
    def getWeightedVotes(self,iis):
        ...
        """
        scores = np.array(list(map(partial(self.calcScore),iis)))
